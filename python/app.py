...
__MODEL = 'random-forest.arroway-d_7-woods.grid.10x10-100x100.4-even.100-est.pkl'
...
start = time.time()
print('loading model')
_MODEL = TextureClassifier(os.path.join(__ROOT, 'database'), __MODEL)
print('model loaded')
...
def predict(fullpath):
	if isinstance(fullpath, bool):
		...
		x = {'results': _MODEL.predict(fullpath)}
		print(x)
		return x
